...
setwd("D:/thera bank solution/")
getwd() 

### Import .xlsx file (given) into the global environment
library(readxl)


### Import given data in 
...
## Missing values
plot_missing(rdata)
# Family Members variable has 0.36% missing values

summary(fivenum(rdata$Family_members))
...
# Convert ZIP Code, Education, Personal loan, Securities account, CD account, Online and Credit Card into factor variables
fac <- c(4,7,9:13)
cdata[,fac] <- lapply(cdata[,fac], as.factor)


# Add labels to numeric variables: Education (ordinal data)
levels(cdata$Education)
summary(cdata$Education)
cdata$Education = factor(cdata$Education, labels = c("Undergrad", "Graduate", "Professional"), order = TRUE)
summary(cdata$Education)
...
# Find missing values
colSums(is.na(cdata))
# there are 18 missing values in variable Family_members
...
# Use mice package to replace negative values
...
# Setting up the graph aesthetics
...
# Majority of customers had 1 or 2 or 3 family members 
# CC_Avg is highly right skewed. 
# 50% customers have a Credit card monthly spend below 1.5K 
...
# Customers with higher Credit card average spend per month have higher liklihood of taking loan.
...
# Customers with CD accounts have greater liklihood of taking loan.
...
# 10 is the optimal number of clusters
...
## CART model 6 important variables
set.seed(seed)
...
# If we launch campaign targetting people with prob > 0.2462, 90.67 % of them have target = 1 or will respond



# response rate at a threshold of 70%
threshold_70 = 0.7
